...
int main()
{
 ...
 //**********************************************************
	...
	wchar_t cw = L'Ф';
	size_t n = sizeof(cw); //unsigned short 2
...
// **************************************************************
	...
	iInt=1;
	double dDouble1=iInt/3;		// (1) неявное приведение результата деления к double
	double dDouble2=iInt/3.;	// (2) неявное приведение числителя и знаменателя в double
	stop
	...
	cByte=sShort=iInt=fFloat=dDouble=3.3/3;			// 1
	...
	iInt	=	257;
	cByte	=	iInt; // переполнение char (ответ 1, тк 257 = 0x0101)
...
// *******************************************************
	...
	// Напишите:
	//   а) как изменилось поведение пременной nStatic1? -> всегда 101
	//   б) как эта переменная ИНИЦИАЛИЗИРОВАНА ? -> 0
	for (int i=0; i<3; i++)
	{
...
// *******************************************************
...

	{
		...
		//ошибки при компиляции 
			...
			//const int nN; // обязательна инициализация
			//nN = 1; // нельзя изменять
	}
...
// ********************************************************
	...
	//Предусмотрите защиту от ввода нижней границы больше, чем верхней.
	{
		...
		std::cout << "Enter min and max limit seq:\n";
		while (std::cin>>min>>max) {
			if ((min >= 0 && min <= 10) && (max >= 0 && max <= 10) && min < max) break;
		}
		// вычислить сумму
	...
	{
		std::cout << "Enter val >= 10 and even:\n";
		int val = 0;

		do {
			std::cin >> val;
		} while ((val < 10) || (val % 2 != 0));
	}
